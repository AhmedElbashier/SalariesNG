...
import { BooksAndResearch,SettingsService } from 'src/app/services/settings.service';
...
deleteBooksAndResearchs(BooksAndResearchs: BooksAndResearch) {
    this.confirmationService.confirm({
        message: 'هل انت متأكد من أنك تريد حذف الحافز  ' + BooksAndResearchs.name + '؟',
        header: 'تأكيد  ',
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
            this.BooksAndResearchs = this.BooksAndResearchs.filter(val => val.id !== BooksAndResearchs.id);
            this.settingService.deleteBooksAndResearch(BooksAndResearchs.id);
            this.BooksAndResearch = {};
            this.messageService.add({ severity: 'success', summary: 'تم بنجاح', detail: 'تم حذف الحافز', life: 3000 });
...
editBooksAndResearchsD(BooksAndResearchs: BooksAndResearch) {
    this.settingService.editBooksAndResearch(BooksAndResearchs);
    this.messageService.add({ severity: 'success', summary: 'تم بنجاح', detail: 'تمت تعديل الحافز بنجاح', life: 3000 });
...
saveBooksAndResearchs(BooksAndResearchs: BooksAndResearch) {
    this.submitted = true;
    this.settingService.addBooksAndResearch(BooksAndResearchs);
...
//             doc.autoTable(this.exportColumns, this.BooksAndResearchss);
//             doc.save('BooksAndResearchss.pdf');
//         })
//     })
// }
...
exportExcel() {
    ...
    import(xlsx).then(xlsx => {
        ...
        const workbook = { Sheets: { 'حوافز الكتب والبحوث': worksheet }, SheetNames: ['حوافز الكتب والبحوث'] };
...
reloadCurrentRoute() {
    ...
    //     this.router.navigate(['dashboard/settings/BooksAndResearchss']);
    // });
  }
}