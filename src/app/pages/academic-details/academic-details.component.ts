...
export class AcademicDetailsComponent {
  ...
  editEmpD(Emp: Emp) {
      ...
      this.messageService.add({ severity: 'success', summary: 'تم بنجاح', detail: 'تمت تعديل بيانات العقار بنجاح', life: 3000 });
      this.Emp = {};
...
deleteDetail(detail: Emp) {
    this.confirmationService.confirm({
        ...
        accept: () => {
            ...
            this.detail = {};
            this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'تم حذف بيانات الموظف باسم', life: 3000 });
            this.reloadCurrentRoute();
        }
    });
}
...
editBuildingD(emp: Emp) {
    this.submitted = true;
    this.EmpService.editEmp(emp);
    this.messageService.add({ severity: 'success', summary: 'تم بنجاح', detail: 'تمت تعديل بيانات الموظف بنجاح', life: 3000 });
    this.detail = {};
    this.reloadCurrentRoute();
}
...
saveDetail(detail: Emp) {
  ...
  this.messageService.add({ severity: 'success', summary: 'تم بنجاح', detail: 'تمت اضافة وحدة العقار بنجاح', life: 3000 });
  this.detail = {};
  this.reloadCurrentRoute();
}
createId(): string {
  ...
  exportExcel() {
    const newLocal = "xlsx";
      import(newLocal).then(xlsx => {
        ...
        const workbook = { Sheets: { 'وحدات البناية': worksheet }, SheetNames: ['وحدات البناية'] };
        const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
        this.saveAsExcelFile(excelBuffer, "وحدات البناية");
